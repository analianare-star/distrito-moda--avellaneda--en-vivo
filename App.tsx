...
const App: React.FC = () => {
  ...
  const [loginStep, setLoginStep] = useState<'ENTRY' | 'AUDIENCE' | 'SHOP' | 'CLIENT' | 'CLIENT_EMAIL'>('ENTRY');
  const [clientEmailMode, setClientEmailMode] = useState<'REGISTER' | 'LOGIN'>('REGISTER');
  ...
  const openLoginModal = (step: 'ENTRY' | 'AUDIENCE' | 'SHOP' | 'CLIENT' | 'CLIENT_EMAIL' = 'ENTRY') => {
      setLoginMode('GOOGLE');
      setLoginStep(step);
  ...
  const handleEmailRegister = async () => {
      ...
      try {
          ...
          const credential = await createUserWithEmailAndPassword(auth, email, loginPassword);
          ...
          setNotice({
              ...
              message: 'Te enviamos un enlace para verificar tu cuenta. Mirá también Spam/Promociones.',
              tone: 'success',
          });
      } catch (error: any) {
          const code = error?.code || '';
  ...
  const bottomNavItems = isAdminUser
      ...
      : isMerchantUser
      ? [
          { id: 'resumen', label: 'Resumen', icon: Home, isCenter: false, onSelect: () => handleMerchantNav('resumen') },
          { id: 'redes', label: 'Redes', icon: Globe, isCenter: false, onSelect: () => handleMerchantNav('redes') },
          { id: 'vivos', label: 'Vivos', icon: Radio, isCenter: true, onSelect: () => handleMerchantNav('vivos') },
          { id: 'reels', label: 'Reels', icon: Film, isCenter: false, onSelect: () => handleMerchantNav('reels') },
          { id: 'perfil', label: 'Perfil', icon: Store, isCenter: false, onSelect: () => handleMerchantNav('perfil') }
        ]
      : [
            ...
            {loginStep === 'CLIENT_EMAIL' && (
              ...
              >
                ...
                <button
                  type="button"
                  onClick={() =>
                    setClientEmailMode((prev) => (prev === 'REGISTER' ? 'LOGIN' : 'REGISTER'))
                  }
                  className="w-full text-center text-[10px] font-semibold text-gray-500 hover:text-dm-dark"
                >
          ...
          {bottomNavItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`flex w-full min-h-[44px] flex-col items-center gap-1 text-[11px] font-semibold ${item.isCenter ? '-translate-y-4' : ''} ${isActive ? 'text-dm-crimson' : 'text-gray-500'}`}
              >
                <span className={`relative flex h-12 w-12 items-center justify-center rounded-full ${item.isCenter ? 'bg-dm-crimson text-white shadow-lg shadow-dm-crimson/30 ring-4 ring-white' : 'bg-white'}`}>
            ...
            <main className="max-w-7xl mx-auto px-4 py-10">
              ...
              {activeBottomNav === 'account' && (
                 <div className="max-w-xl mx-auto rounded-2xl border border-gray-100 bg-white p-6 text-center shadow-sm hidden md:block">
                   ...
                   {user.isLoggedIn ? (
                     <>
                       <p className="mt-2 text-sm text-gray-500">Sesión activa como</p>
                       <p className="mt-1 text-sm font-bold text-dm-dark">{user.name || user.email}</p>
                       <Button className="mt-5" onClick={handleToggleClientLogin}>
                         Cerrar sesión
                       </Button>
                     </>
                   ) : (
            ...
            <div className="flex-1 overflow-y-auto p-6 pb-[calc(1.5rem+env(safe-area-inset-bottom))] space-y-5">
              {user.isLoggedIn ? (
                ...
                <div className="rounded-xl border border-gray-100 bg-gray-50 p-4 text-center">
                  ...
                  <Button className="mt-4 w-full" onClick={openAudienceSelection}>
                    Ingresar
                  </Button>
                  <button
                    onClick={openClientRegister}
