...
import { addFavorite, removeFavorite, addReminder, removeReminder } from "../../../domains/clients";
import { reportStream, rateStream, toggleLikeStream } from "../../../domains/streams";
import { markNotificationRead, markAllNotificationsRead } from "../../../domains/notifications";
import { NotificationItem, Shop, Stream, StreamStatus, UserContext } from "../../../types";
import { AuthProfile } from "../../../hooks/useAuthState";
...
export const useClientActions = ({
  ...
  const handleSubmitReport = useCallback(
    async (reason: string) => {
      ...
      const result = await reportStream(reportTarget.id, reason);
      if (result.success) {
        setUser((prev) => ({
          ...prev,
          reports: [
      ...
      void (async () => {
        try {
          const updated = user.reminders.includes(streamId)
            ? await removeReminder(streamId)
            : await addReminder(streamId);
          if (updated) {
  ...
  const handleMarkAllNotificationsRead = useCallback(() => {
    ...
    void (async () => {
      await markAllNotificationsRead(authProfile.authUserId);
  ...
  const handleRateStream = useCallback(
    (streamId: string, rating: number) => {
      ...
      void (async () => {
        try {
          await rateStream(streamId, rating);
          setNotice({
