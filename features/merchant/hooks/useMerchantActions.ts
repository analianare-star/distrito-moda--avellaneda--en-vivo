...
import { createStream, updateStream, cancelStream } from "../../../domains/streams";
import { updateShop, buyQuota } from "../../../domains/shops";
...
export const useMerchantActions = ({
  ...
  const handleStreamDelete = useCallback(
    async (streamId: string) => {
      ...
      try {
        await cancelStream(streamId, "Cancelado por tienda");
        await refreshData();
      } catch (error) {
  ...
  const handleBuyQuota = useCallback(
    async (amount: number) => {
      ...
      try {
        const result = await buyQuota(currentShopId, amount);
